...
export class ScraperService {
  ...
  async consultarFerrovalle(
    ...
    try {
      ...
      const user = this.configService.get<string>('FERROVALLE_USER');
      const password = this.configService.get<string>('FERROVALLE_PASSWORD');

      if (!user || !password) {
        throw new InternalServerErrorException(
          'Credenciales de Ferrovalle no encontradas en el archivo .env',
        );
      }
